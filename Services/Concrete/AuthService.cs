...
public partial class AuthService : IAuthService
{
    ...
    private async Task<bool> ValidateToken(string token, Guid userId)
    {
    ...
    public async Task<TokenRefreshResult> Refresh(string RefreshToken, Guid userId)
    {
        var validationResult = await ValidateToken(RefreshToken, userId);
        if (validationResult)
        {
            var user = await _authDal.GetAsNoTrackingAsync(u => u.Id == userId);

            string token = _tokenProvider.Create(user!);
            return new TokenRefreshResult(userId, token, true);
        }
        else
        {
            return new TokenRefreshResult(Guid.Empty, "", false);
        }
    }
